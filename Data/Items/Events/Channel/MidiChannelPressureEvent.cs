...
namespace JeffBourdier
{
    ...
    public class MidiChannelPressureEvent : MidiChannelEvent
    {
        ...
        public MidiChannelPressureEvent(MidiTrackChunk owner, int deltaTime, uint channel, byte[] bytes, int index)
            : base(owner, deltaTime, 0xD, channel, 2, Properties.Resources.ChannelPressure) { this.Initialize(bytes, index); }
        ...
        /// <param name="bytes">Array of bytes containing the event data (not including the delta-time or status byte).</param>
        /// <param name="index">Index in the byte array at which the event data begins.</param>
        public MidiChannelPressureEvent(MidiTrackChunk owner, int deltaTime, byte[] bytes, int index)
