...
namespace JeffBourdier
{
    ...
    public class MidiNoteOffEvent : MidiChannelEvent
    {
        ...
        public MidiNoteOffEvent(MidiTrackChunk owner, int deltaTime, uint channel, byte[] bytes, int index)
            : base(owner, deltaTime, 0x8, channel, 3, Properties.Resources.NoteOff) { this.Initialize(bytes, index); }
        ...
        /// <param name="bytes">Array of bytes containing the event data (not including the delta-time or status byte).</param>
        /// <param name="index">Index in the byte array at which the event data begins.</param>
        public MidiNoteOffEvent(MidiTrackChunk owner, int deltaTime, byte[] bytes, int index)
        ...
        {
            this.DataComment = string.Format("{0} | {1,3}", this.NumberToNote(this.NoteNumber, true), this.Velocity);
            this.SetComment();
        }
