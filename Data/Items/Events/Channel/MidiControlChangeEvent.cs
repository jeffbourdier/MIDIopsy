...
namespace JeffBourdier
{
    ...
    public class MidiControlChangeEvent : MidiChannelEvent
    {
        ...
        public MidiControlChangeEvent(MidiTrackChunk owner, int deltaTime, uint channel, byte[] bytes, int index)
            : base(owner, deltaTime, 0xB, channel, 3, Properties.Resources.ControlChange) { this.Initialize(bytes, index); }
        ...
        /// <param name="bytes">Array of bytes containing the event data (not including the delta-time or status byte).</param>
        /// <param name="index">Index in the byte array at which the event data begins.</param>
        public MidiControlChangeEvent(MidiTrackChunk owner, int deltaTime, byte[] bytes, int index)
